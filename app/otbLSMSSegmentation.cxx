...
namespace Wrapper
{
class LSMSSegmentation : public MPIApplication
{
public:
  ...
  itkTypeMacro(LSMSSegmentation, otb::MPIApplication);
...
private:
  ...
  void DoExecute()
  {
    ...
    if(IsParameterEnabled("tmpdir") && this->GetMyRank() == 0)
      {
    ...
    std::vector<unsigned int> tileShifts(nbTilesX*nbTilesY, 0);
    for(unsigned int row = 0; row < nbTilesY; ++row)
      for(unsigned int column = 0; column < nbTilesX; ++column)
        {
        if (currentTileIndex % ((unsigned int) this->GetNProcs()) == ((unsigned int) this->GetMyRank()) )
          {
...
//          regionCount+=currentTileIndex*sizeX*sizeY;
          tileShifts[nbTilesX*row+column]=stats->GetMaximum();

//          std::string filename = WriteTile(labelShiftFilter->GetOutput(),row,column,"SEG");
          std::string filename = WriteTile(ccFilter->GetOutput(),row,column,"SEG");
          }
        currentTileIndex++;
        }
    ...
    // Update tileShifts
    unsigned int nValues = nbTilesX*nbTilesY;
    std::vector<unsigned int> allTileShifts(this->GetNProcs()*nValues);
    MPI_Gather(&(tileShifts[0]),
        nValues,
        MPI_UNSIGNED_LONG,
        &(allTileShifts[0]),
        nValues,
    ...
    if (this->GetMyRank() == 0)
      ...
      for(unsigned int row = 0; row < nbTilesY; ++row)
        for(unsigned int column = 0; column < nbTilesX; ++column)
          {
          unsigned int idx = nbTilesX*row+column;
          unsigned int slaves_max_tileshift = 0;
          for (unsigned int i = 0; i < allTileShifts.size(); i+=nValues)
            {
            slaves_max_tileshift = std::max(slaves_max_tileshift, allTileShifts[i+idx]);
            }
          tileShifts[idx] = std::max(tileShifts[idx], slaves_max_tileshift);
          }

      }
    // BCast tileshifts
    MPI_Bcast( &(tileShifts[0]), tileShifts.size(), MPI_UNSIGNED_LONG, 0, MPI_COMM_WORLD);

    // Wait all processes
    MPI_Barrier(MPI_COMM_WORLD);

    // Update regionCount
    std::vector<unsigned int> newTileShifts(nbTilesX*nbTilesY, 0);
    for (unsigned int i = 0 ; i < tileShifts.size() ; i++)
      {
      if (i>0)
        newTileShifts[i] = regionCount;
      regionCount+=tileShifts[i];
      }

    // Update regionCount
//   std::vector<unsigned int> vRegionCount(this->GetNProcs());
//   MPI_Gather(vRegionCount,
//              1,
//              MPI_UNSIGNED_LONG,
//              &regionCount),
//              1,
      ...
      for(unsigned int column = 0; column < nbTilesX; column++)
        ...
        if (currentTileIndex % ((unsigned int) this->GetNProcs()) == ((unsigned int) this->GetMyRank()) );
          ...
          if(row>0)
            ...
            //Shifting
            LabelShiftFilterType::Pointer labelShiftFilter = LabelShiftFilterType::New();
            labelShiftFilter->SetInput(tileUpReader->GetOutput());
            labelShiftFilter->GetFunctor().SetA(1);
            labelShiftFilter->GetFunctor().SetB(newTileShifts[nbTilesX*row+column]);
            labelShiftFilter->Update();
            ...
            for(pixelIndexIn[0]=0; pixelIndexIn[0]<static_cast<long>(sizeX-1); ++pixelIndexIn[0])
              ...
              LabelImagePixelType adjCanLabel = labelShiftFilter->GetOutput()->GetPixel(pixelIndexUp);
...
OTB_MPI_APPLICATION_EXPORT(LSMSSegmentation)
